...
const ESTOQUE_API_URL =
  "https://script.google.com/macros/s/AKfycbxOy9ngHSvcY_LkngnJwgXu9bzzGJZhZM_0-LwRfF5q-eGuMHOrySLmON3M_swCPxXhxg/exec";
...
function driveImg(url) {
  ...
  if (!match) return url;
  // Corrigido: Removido o '0' extra e corrigida a interpolação
    ...
    div.innerHTML = `
      ...
      item.img,
            ...
            <div class="flex-1 min-w-0 flex flex-col justify-center">
                <h4 class="text-[11px] font-bold text-slate-700 leading-tight truncate mb-0.5">${item.name
      }</h4>
        ...
        .replace(".", ",")}</p>
                    <div class="flex items-center gap-2 bg-slate-50 px-2 py-1 rounded-lg border border-slate-100">
                        <button onclick="changeQty('${item.id
      }', -1)" class="text-[10px] font-bold text-primary px-1">-</button>
                        <span class="text-[10px] font-black min-w-[10px] text-center">${item.quantity
      }</span>
                        <button onclick="changeQty('${item.id
      }', 1)" class="text-[10px] font-bold text-primary px-1">+</button>
                    </div>
...
async function loadProducts() {
  try {
    ...
    allProducts = rows
      ...
      .filter((p) => p["Nome do Produto"]); // Garante que não pegue linha vazia

    console.log("Produtos carregados com sucesso:");
    console.table(allProducts); // Isso mostra se os dados chegaram no F12
...
function addToCart(id, name, price, img) {
      ...
      const originalProduct = allProducts.find(
        (p) => p["ID"].toString() === baseId,
      );
      const estoqueDisponivel = originalProduct
        ? parseInt(originalProduct["Saldo Estoque"])
        : 0;

      // Isso impede que o cliente compre 2 P e 2 M se você só tem 3 no total.
      const totalDesteProdutoNoCarrinho = cart
        .filter(
          (item) =>
      ...
      // 3. Validar se o TOTAL no carrinho já atingiu o estoque da planilha
      if (totalDesteProdutoNoCarrinho >= estoqueDisponivel) {
        Toastify({
          text: `Ops! Limite de estoque atingido (${estoqueDisponivel} unidades).`,
          duration: 3000,
          gravity: "bottom",
          position: "right",
      ...
      // 4. Lógica de inserção (aqui usamos o ID completo com cor/tamanho)
...
const bar = document.getElementById("free-shipping-bar");
  ...
  function openSizeSelector(id, name, price, img) {
    const baseId = id.toString().split("-")[0];
    const p = allProducts.find((prod) => prod["ID"].toString() === baseId);
    if (!p) return;
    ...
    if (temCores) {
      p["Cores"].split(",").forEach((cor) => {
        ...
        btn.style.backgroundColor = coresMap[corNome] || "#cbd5e1";
        btn.onclick = () => selectColor(corNome, p);
    ...
    // 2. Prepara as Opções (ML para garrafas ou Tamanho para roupas)
    const optionsContainer = document.getElementById("options-container");
    optionsContainer.innerHTML = "";

    // Pega os tamanhos da coluna "Tamanho" OU os ML da coluna "ML"
    let opcoes = [];
    if (p["Tamanho"] && p["Tamanho"].trim() !== "") {
      opcoes = p["Tamanho"].split(",");
    } else if (p["ML"] && p["ML"].trim() !== "") {
      opcoes = p["ML"].split(",");
    } else {
      opcoes = ["Único"]; // Caso não tenha nada preenchido
    }

    opcoes.forEach((opt) => {
      const valor = opt.trim();
      const btn = document.createElement("button");
      btn.className =
        "py-3 border rounded-xl font-bold hover:bg-primary hover:text-white transition text-xs";
      btn.innerText = valor;
      btn.onclick = () => finishSelection(valor);
      optionsContainer.appendChild(btn);
    });

    // Se não tiver cores, pula direto para as opções (ML/Tamanho)
    if (!temCores) {
      selectedColor = "Única";
      document.getElementById("color-step").classList.add("hidden");
      document.getElementById("size-step").classList.remove("hidden");
      document.getElementById("modal-step-title").innerText = "Selecione a Opção";
    } else {
  ...
  function selectColor(cor, produtoOriginal) {
    selectedColor = cor;
    document.getElementById("color-step").classList.add("hidden");
    document.getElementById("size-step").classList.remove("hidden");

    // Muda o texto se for garrafa
    const label = document.getElementById("size-label-text");
    if (produtoOriginal["Categoria"].toLowerCase().includes("garrafa")) {
      label.innerText = "Escolha a Capacidade:";
      document.getElementById("modal-step-title").innerText =
        "Selecione o Volume";
    } else {
      label.innerText = "Escolha o Tamanho:";
      document.getElementById("modal-step-title").innerText =
        "Selecione o Tamanho";
    }
  }

  // 2. Quando clica no botão P, M, G ou GG
  function finishSelection(tamanho) {
    console.log("Tentando finalizar com tamanho:", tamanho);

    if (!tempProduct) {
      alert("Erro: Produto não identificado. Tente novamente.");
      closeSizeModal();
      return;
    }

    // Se por algum motivo a cor não foi selecionada (ex: produto sem cor), define como Única
    const corFinal = selectedColor || "Única";

    // Criamos o ID e o Nome que aparecerão no carrinho e no WhatsApp
    const uniqueId = `${tempProduct.id}-${corFinal}-${tamanho}`;
    const fullName = `${tempProduct.name} - ${corFinal} (${tamanho})`;

    // Adiciona ao carrinho usando sua função que já funciona
  ...
  // 3. Função para fechar e LIMPAR tudo
  function closeSizeModal() {
    const modal = document.getElementById("size-modal");
    modal.classList.add("hidden");
    modal.classList.remove("flex");

    // Reseta as variáveis para não misturar no próximo produto
    selectedColor = "";
    tempProduct = null;

    // Volta o título ao original
    document.getElementById("modal-step-title").innerText = "Selecione a Cor";
  }
...
fetch(ESTOQUE_API_URL, {
  ...
  // Atualize o listener dos botões de filtro
  document.querySelectorAll(".filtro-menu-btn, .filtro-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (categoriaAlvo === "todos") {
        ...
        produtosFiltrados = allProducts.filter((p) => {
          ...
          const nomePlanilha = normalizarParaBusca(p["Nome do Produto"]);

          // Lógica para Garrafas: busca na categoria OU no nome do produto
          if (categoriaAlvo === "garrafas") {
            return (
              categoriaPlanilha.includes("garrafa") ||
              categoriaPlanilha.includes("termica") ||
              nomePlanilha.includes("garrafa")
            );
          }

          // Outras categorias (Feminino, Masculino, Acessórios)
      ...
      // Rola para a seção
      const sectionProdutos = document.getElementById("produtos");
      if (sectionProdutos) sectionProdutos.scrollIntoView({ behavior: "smooth" });
    });
  });
