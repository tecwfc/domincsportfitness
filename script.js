    ...
    products.forEach((p) => {
        ...
        card.innerHTML = `
            <div class="relative aspect-[4/5] bg-slate-50 overflow-hidden">
                <img src="${p["Imagem"]}" class="w-full h-full object-cover group-hover:scale-110 transition duration-500 ${isEsgotado ? 'grayscale opacity-50' : ''}">
            ...
            <div class="p-4 flex flex-col flex-1">
                ...
                <h3 class="font-bold text-sm text-textDark leading-tight mb-2 h-10 overflow-hidden">${p["Nome do Produto"]}</h3>
                <div class="mt-auto">
                    ...
                    <button onclick="${clickAction}" ${isEsgotado ? 'disabled' : ''} 
                        ...
                        ${isEsgotado ? 'bg-slate-100 text-slate-400' : 'bg-primary text-white hover:bg-accent hover:shadow-md'}">
                        ${btnText}
                    </button>
                </div>
            </div>
        ...
        if (categoriaSelecionada === "todos") {
            ...
            const filtered = allProducts.filter(p => {
                ...
                if (categoriaSelecionada === "garrafas") {
                    return catProduto.includes("garrafa") || catProduto.includes("termica");
                }
